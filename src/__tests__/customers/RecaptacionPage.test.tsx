...
// recapture-assignable-roles: the assignee pool is every ACTIVE user WITH ≥1
// role AND none technical. Only `tecnico` is barred (noc/admin/ventas qualify).
...
// Pool predicate: ACTIVE + ≥1 role + none technical.
...
// A disabled RbacUser (with a valid role) — must NEVER show up in the operator
...
describe('RecaptacionPage — admin (recapture.assign)', () => {
  ...
  it('A12 — technicians and role-less users are excluded from the bulk pool; admin is INCLUDED', async () => {
    const user = userEvent.setup();
    mockHooks({
      leads: [lead('a')],
      rbacUsers: [...RBAC_USERS, ADMIN_USER, TECNICO_USER, NO_ROLE_USER],
    });
    renderPage();
  ...
  it('A13 — technicians and role-less users are excluded from the inline pool; admin is INCLUDED', () => {
    mockHooks({
      leads: [lead('a')],
      rbacUsers: [...RBAC_USERS, ADMIN_USER, TECNICO_USER, NO_ROLE_USER],
    });
  ...
  it('A14 — a lead assigned to a user OUTSIDE the pool (a technician) still shows their name (phantom intact)', () => {
    // The lead is assigned to a technician, who is NOT in the operator pool. The
    // inline select must still reflect the real assignee via the phantom
    // <option> — the filter must not erase history.
    mockHooks({
      leads: [{ ...lead('a'), assigneeId: 'op-tec', assigneeName: 'Operador Técnico' }],
      rbacUsers: [...RBAC_USERS, TECNICO_USER],
    });
    renderPage();

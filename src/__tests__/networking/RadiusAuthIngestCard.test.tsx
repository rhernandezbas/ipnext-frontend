/**
 * RadiusAuthIngestCard tests
 *
 * Clon de RadiusAccountingCard.test.tsx pero para el flag `radius-auth-ingest`
 * (ingesta de errores de auth → tab "Errores de auth").
 *
 * Covers:
...
function setupHooks({
  ...
  vi.mocked(useFeatureFlag).mockReturnValue({
    data: flagLoading ? undefined : { key: 'radius-auth-ingest', enabled: flagEnabled },
...
describe('RadiusAuthIngestCard', () => {
  ...
  it('renders card title', () => {
    setupHooks();
    render(<RadiusAuthIngestCard />);
    expect(screen.getByRole('heading', { name: /errores de auth radius/i })).toBeInTheDocument();
  });
  ...
  it('clicking the toggle calls mutate with enabled: true when flag is OFF', () => {
    ...
    expect(mutateFn).toHaveBeenCalledWith({ key: 'radius-auth-ingest', enabled: true });
  });
  ...
  it('clicking the toggle calls mutate with enabled: false when flag is ON', () => {
    ...
    fireEvent.click(screen.getByRole('checkbox'));
    expect(mutateFn).toHaveBeenCalledTimes(1);
    expect(mutateFn).toHaveBeenCalledWith({ key: 'radius-auth-ingest', enabled: false });
  });
  ...
  it('toggle is NOT rendered when user has unrelated permissions but not admin.flags', () => {
    setupHooks({ permissions: ['uisp.read', 'network.read'] });
    render(<RadiusAuthIngestCard />);
    expect(screen.queryByRole('checkbox')).not.toBeInTheDocument();
  });

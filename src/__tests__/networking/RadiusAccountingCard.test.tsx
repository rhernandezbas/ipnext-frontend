...
function setupHooks({
  ...
  vi.mocked(useFeatureFlag).mockReturnValue({
    data: flagLoading ? undefined : { key: 'radius-accounting-ingest', enabled: flagEnabled },
...
describe('RadiusAccountingCard', () => {
  ...
  it('clicking the toggle calls mutate with enabled: true when flag is OFF', () => {
    ...
    expect(mutateFn).toHaveBeenCalledWith({ key: 'radius-accounting-ingest', enabled: true });
  });
  ...
  it('clicking the toggle calls mutate with enabled: false when flag is ON', () => {
    ...
    fireEvent.click(screen.getByRole('checkbox'));
    expect(mutateFn).toHaveBeenCalledTimes(1);
    expect(mutateFn).toHaveBeenCalledWith({ key: 'radius-accounting-ingest', enabled: false });
  });

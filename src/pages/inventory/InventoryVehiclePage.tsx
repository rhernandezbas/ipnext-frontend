...
import { useVehicleStock } from '@/hooks/useVehicles';
...
/**
 * Stock de una camioneta: equipos y materiales en su ubicación CAMIONETA
 * (EPIC #38, Wave 5b). El id viene de la URL.
 *
 * Espejo de InventoryTechnicianPage: misma estructura, mismos empty states,
 * mismo botón "Asignar stock" gateado por `inventory.write`.
 * El modal es AssignStockToVehicleModal (sibling, NO toca el modal de técnicos).
 */
export default function InventoryVehiclePage() {
  const { id = '' } = useParams<{ id: string }>();
  const { can } = useMyPermissions();
  const { data, isLoading, isError } = useVehicleStock(id);
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headingGroup}>
          ...
          <h1 className={styles.title}>Stock de la camioneta</h1>
          <p className={styles.subtitle}>
            Equipos y materiales que tiene asignados esta camioneta ahora mismo.
          </p>
        </div>
        {canIssue && (
          <button
      ...
      {isError && (
        <p className={styles.errorBanner} role="alert">
          No pudimos cargar el stock de la camioneta. Reintentá en unos segundos.
        </p>
      )}

      <section className={styles.section} aria-labelledby="vehicle-assets-heading">
        <div className={styles.sectionHead}>
          <h2 id="vehicle-assets-heading" className={styles.sectionTitle}>
            Equipos asignados
          </h2>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            body={
              canIssue
                ? 'Esta camioneta todavía no tiene equipos. Usá "Asignar stock" para entregarle equipos desde el depósito.'
                : 'Esta camioneta todavía no tiene equipos. Se le asignan desde el depósito por una persona con permiso de inventario.'
            }
          />
        ) : (
      ...
      <section className={styles.section} aria-labelledby="vehicle-materials-heading">
        <div className={styles.sectionHead}>
          <h2 id="vehicle-materials-heading" className={styles.sectionTitle}>
            Materiales
          </h2>
  ...
  return (
    <li className={styles.assetRow}>
      ...
      <div className={styles.assetMeta}>
        ...
        <span className={styles.statusPill}>Asignado</span>
      </div>
    </li>
  );
}
